...
pub fn run(policy_path: &str, corpus_path: &str, strict: bool) -> Result<()> {
    ...
    // Report results
    info!("Checks passed: {}/{}", result.checks_passed, result.checks_total);

    for violation in &result.violations {
        error!("[{}] {}: {}", violation.severity, violation.check, violation.message);
    }

    for warning in &result.warnings {
        warn!("[{}] {}: {}", warning.severity, warning.check, warning.message);
    }

    // Determine exit status
    if result.is_rejected() {
        anyhow::bail!("Policy verification failed with {} violation(s)", result.violations.len());
    }

    if strict && !result.warnings.is_empty() {
        anyhow::bail!("Policy verification failed with {} warning(s) (strict mode)", result.warnings.len());
    }
